...
| `http2_max_pending_executions` | `2 * max_threads`                                      | `VAJRA_HTTP2_MAX_PENDING_EXECUTIONS` | `1..2_147_483_647`                                | Per-connection cap for queued and live Rack response producers.                               |
| `http2_max_connection_buffer_bytes` | `16_777_216`                                      | `VAJRA_HTTP2_MAX_CONNECTION_BUFFER_BYTES` | `http2_max_frame_size + 33..2_147_483_647`    | Per-connection Vajra-owned H2 queue ledger; it must hold a DATA frame plus preface/header bytes. |

The H2 buffer ledger covers Vajra-owned queued request data, paused DATA, tunnel buffers, and unsent response bytes. It is not a whole-connection or RSS limit: nghttp2/HPACK state, TLS and kernel buffers, allocator overhead, and the session receive scratch buffer are outside it. A legacy/custom response may also materialize before its later H2 ledger admission, so streamed Rack bodies are the path that receives natural producer backpressure.
