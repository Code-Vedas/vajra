...
namespace Vajra
{
  namespace ipc
  {
    ...
    #define VAJRA_IPC_DEFINE_FRAME_FAMILIES(X) \
      X(request_execution_input, 0x1001, ChannelKind::request) \
      X(request_body_continuation, 0x1002, ChannelKind::request) \
      X(response_metadata_result, 0x1003, ChannelKind::request) \
      X(response_body_continuation, 0x1004, ChannelKind::request) \
      X(protocol_version_negotiation, 0x2001, ChannelKind::control) \
      X(process_registration_identity, 0x2002, ChannelKind::control) \
      X(readiness_boot_result, 0x2003, ChannelKind::control) \
      X(lifecycle_command, 0x2004, ChannelKind::control) \
      X(lifecycle_state_notification, 0x2005, ChannelKind::control) \
      X(diagnostics_error_reporting, 0x2006, ChannelKind::control) \
      X(telemetry_status_reserved, 0x2007, ChannelKind::control)
    ...
    {
#define VAJRA_IPC_DEFINE_FRAME_FAMILY_ENUM(name, wire_id, channel) name = wire_id,
...
#undef VAJRA_IPC_DEFINE_FRAME_FAMILY_ENUM
    ...
    constexpr std::size_t kFrameFamilyCount =
        0
        #define VAJRA_IPC_DEFINE_FRAME_FAMILY_COUNT(name, wire_id, channel) + 1
    ...
    constexpr std::array<FrameFamily, kFrameFamilyCount> kFrameFamilies = {
        #define VAJRA_IPC_DEFINE_FRAME_FAMILY_VALUE(name, wire_id, channel) FrameFamily::name,
    ...
    constexpr std::array<FrameFamilyMetadata, kFrameFamilyCount> kFrameFamilyMetadata = {
        #define VAJRA_IPC_DEFINE_FRAME_FAMILY_METADATA(name, wire_id, channel) FrameFamilyMetadata{FrameFamily::name, channel},
